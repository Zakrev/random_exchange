...
void get_StartData(int argv, char ** args, StartData * sdata)
{
        switch(argv){
                ...
                case 6:
                        /*
                                -new id -users n -days n
                        */
                        if(strcmp("-new", args[1]) == 0 && strcmp("-users", args[3]) && strcmp("-days", args[5])){
                                sscanf(args[2], "%u", &sdata->id);
        ...
        sdata->cmd = 0;
        printf("Need arguments: <-new id | -con id> <-users n> <-days n>\n");
}
